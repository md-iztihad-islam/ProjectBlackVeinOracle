import getCellBlockByIdApi from "@/services/CellBlock/getCellBlockByIdApi";
import updateCellBlockApi from "@/services/CellBlock/updateCellBlockApi";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";

const labelClass = "block text-[10px] tracking-[0.18em] uppercase text-slate-400 mb-2";
const inputClass =
  "w-full bg-slate-900/70 border border-slate-700 text-slate-100 placeholder-slate-500 px-4 py-3 text-sm outline-none focus:border-blue-400/40 hover:border-slate-500 transition-colors appearance-none rounded-xl";
const hintClass = "text-[10px] text-slate-500 mt-1.5";

export default function UpdateCellBlock() {
  const navigate = useNavigate();
  const { cellBlockId } = useParams();
  const queryClient = useQueryClient();

  const { data: cellBlockData, isLoading: cellBlockDataLoading, error: cellBlockDataError } = useQuery({
    queryKey: ["cellBlockById", cellBlockId],
    queryFn: () => getCellBlockByIdApi(cellBlockId),
    enabled: !!cellBlockId,
  });

  const cellBlock = cellBlockData?.data || null;

  const [cellBlockName, setCellBlockName] = useState("");
  const [capacity,      setCapacity]      = useState("");
  const [formError,     setFormError]     = useState("");

  useEffect(() => {
    if (cellBlock) {
      setCellBlockName(cellBlock.block_name || "");
      setCapacity(cellBlock.capacity?.toString() || "");
    }
  }, [cellBlock?.block_id]);

  const { mutate: updateCellBlock, isLoading: updateCellBlockLoading } = useMutation({
    mutationFn: ({ cellBlockId, cellBlockData }) => updateCellBlockApi({ cellBlockId, cellBlockData }),
    onSuccess: () => {
      queryClient.invalidateQueries(["cellBlockData"]);
      queryClient.invalidateQueries(["cellBlockById", cellBlockId]);
      navigate(-1);
    },
    onError: () => setFormError("Failed to update cell block. Please try again."),
  });

  const handleUpdate = () => {
    setFormError("");
    if (!cellBlockName.trim()) {
      setFormError("Block name is required.");
      return;
    }
    if (!capacity || isNaN(parseInt(capacity)) || parseInt(capacity) < 1) {
      setFormError("Capacity must be a positive integer (≥ 1).");
      return;
    }
    updateCellBlock({
      cellBlockId,
      cellBlockData: {
        block_name: cellBlockName.trim(),
        capacity: parseInt(capacity),
      },
    });
  };

  return (
    <div
      className="min-h-screen bg-[#080a0e] text-slate-300 px-6 py-10 md:px-10"
      style={{
        backgroundImage:
          "linear-gradient(rgba(96,165,250,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(96,165,250,0.025) 1px,transparent 1px)",
        backgroundSize: "40px 40px",
        fontFamily: "'IBM Plex Mono', monospace",
      }}
    >
      <div className="max-w-5xl mx-auto">
      <div className="mb-6 flex items-center justify-between gap-3">
        <button
          onClick={() => navigate(-1)}
          className="rounded-xl border border-slate-600 bg-slate-900/70 text-slate-100 px-4 py-2 text-[11px] font-black tracking-widest uppercase hover:border-blue-400/50 hover:text-blue-300 hover:bg-slate-800/80 transition-all"
        >
          ← Back
        </button>
      </div>

      {/* ── Header ── */}
      <div className="mb-10">
        <span className="text-[10px] tracking-[0.22em] uppercase text-blue-400 bg-blue-400/10 border border-blue-400/20 px-3 py-1 inline-block mb-3">
          Edit Record · <span className="text-white">{cellBlockId}</span>
        </span>
        <h1
          className="text-4xl md:text-5xl font-black tracking-widest uppercase text-white leading-none"
          style={{ fontFamily: "'Rajdhani', sans-serif" }}
        >
          Update Cell <span className="text-blue-400">Block</span>
        </h1>
        <p className="text-[11px] text-slate-500 mt-2 tracking-widest">
          // Modify an existing cell block record
        </p>
      </div>

      <div className="h-px bg-gradient-to-r from-blue-400/30 via-blue-400/10 to-transparent mb-10" />

      {/* ── Loading ── */}
      {cellBlockDataLoading && (
        <div className="flex items-center gap-3 py-16 text-slate-500 text-[11px] tracking-widest">
          <div className="w-4 h-4 border border-slate-700 border-t-blue-400 rounded-full animate-spin" />
          LOADING BLOCK DATA...
        </div>
      )}

      {/* ── Fetch Error ── */}
      {cellBlockDataError && (
        <div className="bg-red-500/8 border border-red-500/20 text-red-400 text-[12px] tracking-widest px-4 py-3 mb-6 max-w-xl">
          ⚠ Failed to load block data. Please go back and try again.
        </div>
      )}

      {/* ── Form Error ── */}
      {formError && (
        <div className="bg-red-500/8 border border-red-500/20 text-red-400 text-[12px] tracking-widest px-4 py-3 mb-6 max-w-xl">
          ⚠ {formError}
        </div>
      )}

      {!cellBlockDataLoading && !cellBlockDataError && (
        <div className="max-w-2xl flex flex-col gap-0">
          <div className="text-[10px] tracking-[0.22em] uppercase text-slate-500 pb-3 mb-5 border-b border-slate-700/80">
            // Block Details
          </div>

          <div className="mb-4">
            <label className={labelClass}>Block Name *</label>
            <input
              className={inputClass}
              value={cellBlockName}
              onChange={(e) => setCellBlockName(e.target.value)}
              placeholder="e.g. Block A, North Wing"
              maxLength={100}
            />
            <p className={hintClass}>Must be unique within the facility</p>
          </div>

          <div className="mb-8">
            <label className={labelClass}>Capacity *</label>
            <input
              className={inputClass}
              type="number"
              min={1}
              value={capacity}
              onChange={(e) => setCapacity(e.target.value)}
              placeholder="e.g. 50"
            />
            <p className={hintClass}>Total number of inmates this block can hold</p>
          </div>

          {/* ── Preview ── */}
          {(cellBlockName || capacity) && (
            <div className="bg-blue-400/5 border border-blue-400/15 px-5 py-4 mb-8 flex items-center gap-5">
              <div
                className="w-12 h-12 flex items-center justify-center text-base font-black text-blue-400 bg-slate-800 border border-slate-700 flex-shrink-0"
                style={{ fontFamily: "'Rajdhani', sans-serif" }}
              >
                {capacity || "—"}
              </div>
              <div>
                <div className="text-[10px] text-slate-500 tracking-widest uppercase mb-1">Preview</div>
                <div className="text-white font-bold text-lg" style={{ fontFamily: "'Rajdhani', sans-serif" }}>
                  {cellBlockName || "Block Name"}
                </div>
                <div className="text-[11px] text-blue-400 mt-0.5">
                  ID: {cellBlockId} · Capacity: {capacity || "—"}
                </div>
              </div>
            </div>
          )}

          {/* ── Actions ── */}
          <div className="flex flex-wrap items-center gap-3 pt-6 border-t border-slate-700">
            <button
              onClick={handleUpdate}
              disabled={updateCellBlockLoading}
              className="rounded-xl bg-blue-400 text-[#080a0e] px-8 py-3.5 text-[13px] font-black tracking-widest uppercase hover:bg-blue-300 hover:-translate-y-0.5 disabled:bg-blue-900 disabled:text-blue-700 disabled:translate-y-0 transition-all duration-150"
            >
              {updateCellBlockLoading ? "SAVING..." : "SAVE CHANGES"}
            </button>
            <button
              onClick={() => navigate(-1)}
              className="rounded-xl border border-slate-600 bg-slate-900/70 text-slate-100 px-6 py-3.5 text-[13px] font-bold tracking-widest uppercase hover:border-blue-400/40 hover:text-blue-300 transition-all duration-150"
            >
              CANCEL
            </button>
          </div>
        </div>
      )}
      </div>
    </div>
  );
}