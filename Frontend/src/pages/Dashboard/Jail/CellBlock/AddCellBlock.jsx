import addCellBlockApi from "@/services/CellBlock/addCellBlockApi";
...
export default function AddCellBlock() {
  ...
  const { mutate: addCellBlock, isLoading } = useMutation({
    mutationFn: (data) => addCellBlockApi(data),
    onSuccess: () => {
      queryClient.invalidateQueries(["cellBlockData", jailId]);
      navigate(-1);
    },
    onError: () => setFormError("Failed to add cell block. Please try again."),
  });

  const handleAddCellBlock = () => {
    setFormError("");
    if (!cellBlockName.trim() || !capacity) { setFormError("Block name and capacity are required."); return; }
    if (isNaN(parseInt(capacity)) || parseInt(capacity) < 1) { setFormError("Capacity must be a positive integer (≥ 1)."); return; }
    addCellBlock({ jail_id: jailId, block_name: cellBlockName.trim(), capacity: parseInt(capacity) });
  };
    ...
    <div className="min-h-screen bg-[#080a0e] text-slate-300 px-6 py-10 md:px-10"
      ...
      <div className="max-w-xl">
        ...
        <div className="mb-4">
          ...
          <input className={inputClass} value={cellBlockName} onChange={(e) => setCellBlockName(e.target.value)} placeholder="e.g. Block A, North Wing" />
          <p className={hintClass}>Descriptive name for this housing block</p>
        </div>

        <div className="mb-8">
          <label className={labelClass}>Capacity *</label>
          <input className={inputClass} type="number" min={1} value={capacity} onChange={(e) => setCapacity(e.target.value)} placeholder="e.g. 50" />
          <p className={hintClass}>Total inmate capacity for this block (≥ 1)</p>
        </div>

        {(cellBlockName || capacity) && (
          <div className="bg-blue-400/5 border border-blue-400/15 px-5 py-4 mb-8 flex items-center gap-5">
            <div className="w-12 h-12 flex items-center justify-center text-lg font-black text-blue-400 bg-slate-800 border border-slate-700 flex-shrink-0" style={{ fontFamily: "'Rajdhani', sans-serif" }}>
              {capacity || "—"}
            </div>
            <div>
