...
export default function JailNotificationCenter() {
  ...
  const { data: unreadData } = useQuery({
    queryKey: ["jailNotificationUnreadCount"],
  ...
  const readOneMut = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["jailNotifications"] });
      queryClient.invalidateQueries({ queryKey: ["jailNotificationUnreadCount"] });
    },
  });
                ...
                return (
                  <li key={n.notification_id} className="p-4 flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      {(meta.criminalId || meta.location || meta.escapedFrom) && (
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-xs px-2 py-1 rounded-full ${
                          n.is_read ? "bg-slate-700 text-slate-300" : "bg-amber-500/20 text-amber-300"
                        }`}
                      >
